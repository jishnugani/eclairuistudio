...
export default function Home({onOpen}) {
  ...
  return <div className="home-story" ref={home}>
    <section className="home-section home-profile">
      ...
      <div className="home-profile-grid"><div><h1 className="profile-title" aria-label="I’m ec92a5.">{Array.from('I’m ec92a5.').map((letter,index)=><span aria-hidden="true" className="profile-letter-mask" key={index}><span style={{'--letter':index}}>{letter===' '?'\u00a0':letter}</span></span>)}</h1><p>I’m an independent UI designer with over a year of experience. I create menus, HUDs, shops, and inventories in Figma.</p><div className="profile-styles"><span>Cartoony</span><span>Anime</span><span>Retro</span><span>Minimal</span></div><Contact className="text-link">Contact on Discord <ArrowUpRight size={18}/></Contact></div><dl className="home-profile-facts"><div><dt>Experience</dt><dd>1+ <span>year</span></dd></div><div><dt>Age</dt><dd>18</dd></div><div><dt>Timezone</dt><dd>Pacific Time</dd></div><div><dt>Design tool</dt><dd><Figma size={23}/>Figma</dd></div></dl></div>
    </section>
    ...
    <section className={`home-section home-reviews testimonial-section ${reviewsPaused?'reviews-paused':''}`}><div className="home-section-label">03 / CLIENT REVIEWS</div><div className="home-section-heading"><h2>Client feedback.</h2><button className="icon-button review-motion" aria-label={reviewsPaused?'Play review animation':'Pause review animation'} aria-pressed={reviewsPaused} onClick={()=>setReviewsPaused(!reviewsPaused)}>{reviewsPaused?<Play size={17}/>:<Pause size={17}/>}</button></div><div className="testimonial-window" tabIndex={0} role="region" aria-label="Client reviews"><div className="testimonial-track">{[0,1].map(copy=><div className="testimonial-group" key={copy} aria-hidden={copy===1?true:undefined}>{reviews.map(review=><figure className="testimonial-card" key={review.name}><div className="review-stars" role="img" aria-label={`${review.rating} out of 5 stars`}>{Array.from({length:5},(_,i)=><Star key={i} size={20} fill="currentColor" aria-hidden="true"/>)}</div><blockquote>“{review.quote}”</blockquote><figcaption><strong>{review.name}</strong><span>{review.role}</span></figcaption></figure>)}</div>)}</div></div></section>
